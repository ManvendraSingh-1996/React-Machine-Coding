...
> Tailored to profile: **frontend-heavy React engineer** with data-pipeline Node.js, Azure services, and BI experience.
...
## Section 0 — Positioning Strategy (read this first)

Your profile vs. this JD, honestly assessed:

| JD asks for | You have | Gap size |
|---|---|---|
| Node.js backend (Express, API design, auth) | Real but narrow: ingestion services, per-tenant auth, pagination/retries/rate-limits, JWT/RBAC at the MF host | **Small–medium** — you have stories, you need vocabulary |
| Angular | Used ~2 years ago | **Medium** — refresher needed (Section 4) |
| React Native | **Not on your resume** | **Largest gap** — but React depth is your bridge |
| SQL / NoSQL | Azure SQL, PostgreSQL, schema design for 10M+ records | Strong ✅ |
| Microservices / NestJS | None | Known gap — scripted answers in Sections 10 & 15 |
| Testing, Git, CI/CD, Agile | Jest/RTL, PR reviews, unattended pipelines | Strong ✅ |

**Your three moves in the interview:**

1. **Lead with your strengths, loudly.** You're not a weak full-stack dev — you're a *strong frontend architect* (Module Federation host, 30+ live WebSocket charts, component libraries used across 5+ products) with *production Node.js data engineering*. That combination is rarer than generic full-stack. Frame yourself that way in "tell me about yourself."

2. **Reframe your Node experience — don't undersell it.** You didn't "just do basic Node." You built services that: handled OAuth/token auth against 5 different vendor APIs, implemented retry + rate-limit + pagination logic (that IS resilient API client design), ran unattended with logging and alerting (that IS production operations), and cut Sage Intacct API consumption by 50% (that IS performance optimization). Say it in those words. What you *haven't* done: designed large Express app architectures serving user traffic, WebSocket servers, caching layers. When asked about those, answer conceptually (Sections 1–3 give you the material) and anchor back to what you have shipped.

3. **Bridge to React Native through React.** Expected question: *"Have you built mobile apps?"* Honest answer that lands well:
   > "I haven't shipped a React Native app to production yet. But RN is React — same components, hooks, state management, and the exact performance discipline I already apply: my thermal dashboard renders 30+ live charts over WebSockets using throttling and memoization, which is the same re-render optimization RN demands. The deltas are the native pieces — FlatList virtualization, navigation, platform APIs, the release process — and I've studied those." *(Then let Section 6 answers back you up.)*

   Strongly recommended before the interview: **build one tiny RN app** (even 2–3 evenings — a todo or a list-with-API app via Expo). "I built a small app last week to learn the toolchain" transforms this answer.

**Where the interview will actually spend its time:** at 4–5 yrs experience, expect ~40% of it on *your resume projects*. Section 14.5 (Resume Deep-Dive) is the highest-value section in this file — prepare it first.

---

## Table of Contents

0. [Positioning Strategy](#section-0--positioning-strategy-read-this-first)
...
14.5. [Resume Deep-Dive — Questions About YOUR Projects](#145-resume-deep-dive--questions-about-your-projects) ⭐ prepare this first
...
> **Framing for your profile:** your Node work was data pipelines and API ingestion, not user-facing servers. That's fine — these fundamentals apply to both. When answering, connect concepts to what you did: streams → processing large API payloads; retries/backoff → your vendor API clients; unhandled rejections → your pipeline's failure alerting. Concepts you've only read about (clustering, WebSocket servers), answer conceptually without claiming production use.
...
### Q99. "Tell me about yourself." (prepare a 90-second version — draft below, make it yours)
**A (tailored draft):**
> "I'm a software engineer with 4 years of experience, currently at Deevia Software in Bangalore. My core strength is frontend architecture in React and TypeScript — I lead frontend across our enterprise products, including our flagship Module Federation platform where products plug in as independently deployed remotes with centralized JWT/RBAC auth, and a real-time thermal monitoring dashboard rendering 30+ live charts over WebSockets. On the backend side, I've built Node.js data services integrating five ERP/CRM vendor APIs — per-tenant auth, retries, rate limits — consolidating 10M+ records into Azure SQL for BI dashboards, running unattended in production. I'm looking to go deeper on the full stack — this role's mix of Node backend, web, and mobile is exactly the breadth I want to grow into."

Practice it out loud until it's smooth, not memorized-sounding.
...
## 14.5. Resume Deep-Dive — Questions About YOUR Projects

> ⭐ **Prepare this section first.** At your experience level, interviewers spend heavily on resume projects. Every claim on your resume is a question waiting to happen. Prepare crisp 60–90 second answers with numbers.

### Micro Frontend / Module Federation (they WILL ask — it's your headline project)

**Q. Explain your Module Federation setup. (YOUR rehearsed 90-second answer)**
> "We built a host **'Framework' application** that owns all platform-level concerns once — authentication, user management, departments, roles and access criteria, and application settings. Every product — Tuyere Monitoring, Blast Furnace, and others — is an **independent plugin**: a separate React app exposing its root module via Webpack Module Federation. At runtime the host loads each plugin and passes the signed-in user's context — token, roles, departments, access criteria — **down as props**, so no plugin re-implements auth or user management. Onboarding a new product means dropping the plugin's **build artifact into the host's serving location and registering it** — the host's application code isn't rebuilt. The same framework serves multiple clients: each client is provisioned with only the plugins they've licensed, and within a client, RBAC controls which plugin interfaces each user sees."

Vocabulary upgrade: never say "paste the dist in the main code folder" — say **"drop the build artifact into the host's serving location and register it."**

⚠️ **Consistency check on "zero host redeployment":** know your true answer before the interview. If plugin integration is artifact-copy + config/registry entry → genuinely zero redeploy, say so. If a host rebuild is involved → fallback: *"the host's application code never changes for a new plugin — integration is config-only, which was the win over our previous coupled releases."* Don't get caught overclaiming on your headline project.

**Q. Why Module Federation instead of npm packages or a monorepo?**
**A:** npm packages require every consumer to bump versions and redeploy; a monorepo couples release cycles across teams. Our drivers were **per-client licensing** (provision only licensed modules) and **independent product releases** — plugin teams ship without coordinating a host release.

**Q. How does the host know which plugins to load per client/user?**
**A:** Per-client plugin registry/config (licensing decides what's available), then per-user RBAC check before rendering a plugin's entry point.

**Q. What happens if a plugin fails to load or crashes at runtime?**
**A:** Error boundary + loading/fallback UI around each plugin mount so one broken remote can't take down the host shell. (If you didn't build this, say it's what you'd add — don't claim it.)

**Q. How is data scoped per plugin/client? (your instance-ID answer)**
> "Each plugin instance is provisioned with an **instance ID**, passed in by the host alongside the auth context. Every API call from the plugin carries that instance ID, and the backend uses it to **scope the data** — so the same plugin codebase serves different clients and deployments, each seeing only its own instance's data. A new client instance is a provisioning step, not a code change."

⚠️ **Sharp follow-up to expect:** *"What stops a user from sending someone else's instance ID?"* (IDOR question). Best answer: *"the instance ID is a scoping hint — the backend authorizes it against the user's JWT claims, so users can only query instances they're entitled to."* If your system didn't validate server-side: *"each client ran against their own provisioned backend, so cross-instance access wasn't exposed — in a shared backend I'd validate the instance ID against token claims on every request."* Verify which is true for your system before the interview.

**Q. Why an instance ID instead of deriving scope from the token?**
**A:** One client/user can have multiple plugin instances (e.g., two furnace deployments) — the frontend must say *which* instance it's viewing; the token decides whether they're *allowed* to view it.

**Q. How did shared dependencies work? What problems did you hit?**
**A:** Be ready to talk about `shared` config — singleton React/ReactDOM (two React copies break hooks/context), version mismatch warnings, eager loading vs async boundaries. Have one real war story: e.g., a plugin bundling its own React copy, or a shared library version conflict, and how you resolved it.

**Q. How did auth work across host and plugins?**
**A:** Centralized at the host — JWT authentication and multi-role RBAC live in one place; auth context and tokens are **propagated into plugin instances as props**. Be ready for follow-ups: How do plugins make authenticated API calls (token passed down vs shared axios instance)? What happens on token expiry mid-session (refresh handling)? How does RBAC hide/show plugin features (role checks at host routing level vs inside plugins)? Prepare concrete answers from what you actually built.

**Q. What are the downsides of micro frontends?** *(maturity check)*
**A:** Runtime coupling to the remote's availability, shared-dependency version management, harder E2E testing, CSS/global-state isolation discipline, and organizational overhead — not worth it for a small single-team app. You used it because plugin licensing and independent product releases genuinely required it.

### Real-time / WebSockets (second most likely deep-dive)

**Q. How did you keep the UI responsive with 30+ live charts on WebSocket data?**
**A (structure it):** (1) **Throttled updates** — high-frequency messages are buffered and flushed to state on an interval (e.g., every 250–500ms) instead of one render per message. (2) **Memoized rendering** — each chart wrapped so only the chart whose data changed re-renders (React.memo, stable props, careful selector design). (3) Chart-level tricks — ECharts `setOption` with incremental/merge updates rather than re-initializing, capped data windows (rolling buffers, not unbounded arrays). Know your actual numbers: message frequency, how you chose the throttle interval.

**Q. What happens when the WebSocket disconnects?**
**A:** Have a real answer: reconnect with backoff, connection-status indicator in the UI, and how you handled the data gap (refetch recent history vs accept the gap). If you didn't handle something, say what you'd add.

**Q. WebSockets vs polling vs SSE — why WebSockets here?**
**A:** Polling wastes requests and adds latency; SSE is server→client only; WebSockets give low-latency bidirectional streaming — right for continuous high-frequency thermal/sensor feeds. Bonus: mention you'd consider SSE if the stream were one-directional and infrastructure favored plain HTTP.

### Node.js / Azure Data Platform (your backend credibility — sell it)

**Q. Walk me through the multi-tenant ERP ingestion architecture.**
**A (90 seconds, practice it):** Node.js services integrate 5 vendor platforms (ServiceTitan, Sage Intacct, BuildOps, Azuga, Geotab). Each client is a tenant with its own credentials — services handle **per-tenant auth** (different auth schemes per vendor), **pagination** through large datasets, **retry with backoff** on transient failures, and **rate-limit compliance** per vendor. I analyzed each API to design the consolidated **Azure SQL schema** — 10M+ operational/financial/fleet records into one source of truth — feeding 50+ KPIs in Power BI, auto-distributed via Report Builder and Power Automate. It runs **unattended**: scheduled jobs, per-run logging, email alerts on failure — so expired tokens and vendor API changes surface immediately instead of as silently missing data.

**Q. How did you cut Sage Intacct API consumption by 50%?**
**A:** Know your own answer cold — likely some mix of: incremental/delta queries instead of full pulls, batching, widening page sizes, caching unchanged reference data, smarter scheduling. Whatever you actually did, be specific — this is your best "backend optimization" story.

**Q. How did you handle failures/idempotency in the pipeline?**
**A:** Talk about: what happens if a run dies mid-way (resume vs re-run safety — upserts/MERGE instead of blind inserts), per-run logs for diagnosis, alerting on failure. If asked "idempotency" and you used upserts, that IS idempotent ingestion — use the word.

**Q. Explain the database recovery pipeline.**
**A (60 seconds):** Event-driven, replacing a manual monthly process: a **Logic App** watches an SFTP endpoint → each new backup triggers **ADF** → webhook-invoked **Automation Runbooks** handle transfer, SSMS restore, and migration into Azure SQL → per-run logs to Blob Storage. The design point to emphasize: event-driven instead of scheduled polling, and observable (per-run logs) instead of a black box.

### React performance (they'll ask because your resume claims 50% load-time cut)

**Q. How exactly did you cut page load times by 50%?**
**A:** Be specific and honest about measurement: route-based **code splitting** + **lazy loading** (React.lazy/Suspense) so the initial bundle carries only the current route; **memoization** (React.memo/useMemo/useCallback) to stop cascade re-renders; mention how you measured (Lighthouse, bundle analyzer, before/after numbers). Expect follow-up: "When does useMemo hurt?" — overhead exceeds benefit for cheap computations; premature memoization complicates code.

**Q. How did you design the shared component library used across 5+ products?**
**A:** Prepare: API design philosophy (props vs composition), theming (Material UI theme extension), versioning/distribution, how breaking changes were managed across consumers, and testing strategy (Jest/RTL per component). One concrete example component makes this answer real.

### The honest-gap questions (rehearse these verbatim)

**"Your backend experience looks like data pipelines, not application servers. Can you handle our Node backend work?"**
> "That's fair — my Node experience is integration and data engineering rather than serving user traffic. But the hard parts transfer: I've dealt with auth against five different vendor APIs, resilient HTTP clients with retries and rate limits, schema design for 10M+ records, and running services unattended in production with logging and alerting. The Express request/middleware layer is the part I know more from building against APIs than building them — and that's the most learnable layer. I'd expect to be productive there within weeks, not months."

**"Have you worked with React Native?"** → use the Section 0 bridge answer.

**"Your Angular is 2 years old."**
> "Yes — my recent depth is React, and I used Angular about two years back. The concepts carried over both ways: DI, RxJS patterns, component architecture. I've refreshed on where Angular has moved — standalone components, Signals — and given I work in TypeScript daily, the ramp back is short." *(Do actually spin up a small Angular app before the interview so this is true.)*
...
## Final prep checklist (priority order for YOUR profile)

- [ ] **Section 14.5 first** — 60–90s answers for Module Federation, WebSocket dashboard, ERP pipeline, Sage Intacct optimization, 50% load-time cut. Know your own numbers cold.
- [ ] **Build a tiny React Native app** (2–3 evenings, Expo: list + API + navigation) — converts the biggest JD gap from "no" to "yes, recently"
- [ ] **Refresh Angular hands-on** — spin up one small standalone-component app; skim Section 4
- [ ] **Express muscle memory** — build one small CRUD API (routes/controllers/services, JWT middleware, validation) so Section 2–3 answers come from your fingers, not just this file
- [ ] Memorize the rapid-fire set: debounce/throttle from scratch, event-loop outputs, 2nd-highest-salary SQL, JWT flow
- [ ] Rehearse the three honest-gap answers (backend depth, React Native, Angular recency) until they sound confident, not apologetic
- [ ] Re-read your own repo (Todo, Accordion, parent-child, custom hooks) — walk-throughs of *your* code are likely
- [ ] Prepare 3 STAR stories + the 90-second intro (Q99 draft) + 3 questions to ask them
